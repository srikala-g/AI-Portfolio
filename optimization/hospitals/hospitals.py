"""
Hospital Placement Optimization using Hill Climbing Algorithm

This module implements a hill climbing optimization algorithm to find optimal 
hospital placements in a grid-based space. The goal is to minimize the total 
distance from all houses to their nearest hospital.

Key Features:
- Grid-based space representation with houses and hospitals
- Hill climbing algorithm for local optimization
- Random restart capability for global optimization
- Visual output generation showing the optimization process
- Manhattan distance cost function

Classes:
    Space: Main class for managing the optimization space and algorithms

Usage Instructions:
    1. Create a Space object with dimensions and number of hospitals:
    ...
    3. Run hill climbing optimization:
       hospitals = s.hill_climb(maximum=100, image_prefix="output", log=True)
    
    4. For better results, use random restart:
       hospitals = s.random_restart(maximum=10, image_prefix="output", log=True)
    
    5. The algorithm will generate images showing the optimization process
       and return the optimal hospital positions.

Requirements:
    - PIL (Pillow) for image generation
    - House.png and Hospital.png images in assets/images/
    - OpenSans-Regular.ttf font in assets/fonts/

Example:
    # Create space and add houses
    s = Space(height=10, width=20, num_hospitals=3)
    for i in range(15):
        s.add_house(random.randrange(s.height), random.randrange(s.width))
    
    # Optimize hospital placement
