// *************** Modified by Sukanta on 3rd April //
...
lang : {
	...
	fullExpandTitle : '',
	creditsText : 'iDesigns Creation',
	creditsTitle : 'iDesigns Creation',
...
//creditsHref : 'http://www.idesigns.co.in',
...
wrapperClassName : 'zoomer-wrapper', // for enhanced css-control
skin : {
	controls:
		'<div class="zoomer-controls"><ul>'+
			'<li class="zoomer-previous">'+
				'<a href="#" title="{hs.lang.previousTitle}">'+
				'<span>{hs.lang.previousText}</span></a>'+
			'</li>'+
			'<li class="zoomer-play">'+
				'<a href="#" title="{hs.lang.playTitle}">'+
				'<span>{hs.lang.playText}</span></a>'+
			'</li>'+
			'<li class="zoomer-pause">'+
				'<a href="#" title="{hs.lang.pauseTitle}">'+
				'<span>{hs.lang.pauseText}</span></a>'+
			'</li>'+
			'<li class="zoomer-next">'+
				'<a href="#" title="{hs.lang.nextTitle}">'+
				'<span>{hs.lang.nextText}</span></a>'+
			'</li>'+
			'<li class="zoomer-move">'+
				'<a href="#" title="{hs.lang.moveTitle}">'+
				'<span>{hs.lang.moveText}</span></a>'+
			'</li>'+
			'<li class="zoomer-full-expand">'+
				'<a href="#" title="{hs.lang.fullExpandTitle}">'+
				'<span>{hs.lang.fullExpandText}</span></a>'+
			'</li>'+
			'<li class="zoomer-close">'+
...
{	
	...
	var el = e.target;
	while (el.parentNode
		&& !(/zoomer-(image|move|html|resize)/.test(el.className)))
	{
		el = el.parentNode;
	}
	...
	if (exp && e.type == 'mousedown') {
		...
		var match = el.className.match(/zoomer-(image|move|resize)/);
		if (match) {
			...
			if (/zoomer-(image|html)-blur/.test(exp.content.className)) {
				exp.focus();
				hs.hasFocused = true;
			}
			return false;
		...
		} else if (/zoomer-image-blur/.test(el.className)) {
...
fixCursor: function() {
	...
	for (i = 0; i < ss.length; i++) for (j = 0; j < ss[i].rules.length; j++) {
		rule = ss[i].rules[j];
		if (rule.selectorText == '.zoomer IMG' && re.test(rule.style.cursor)) {
...
hs.Outline =  function (outlineType, onLoad) {
	...
	this.td[4].className = outlineType +' zoomer-outline';
		...
		(function(){
			...
			setTimeout ( function() {
				if (up && pI == 1) {
					...
					exp.a.className += ' zoomer-active-anchor';
				}
				exp.setSize(size);
			}, t);				
		})();
...
crossfadeEnd : function() {
	...
	this.a.className += ' zoomer-active-anchor';
	this.afterExpand();
...
getNumber : function() {
	if (this[this.numberPosition]) {
		...
		this[this.numberPosition].innerHTML = 
			'<div class="zoomer-number">'+ s +'</div>'+ this[this.numberPosition].innerHTML;
	}
},
	...
	for (var i = 0; i < types.length; i++) {
		...
		if (!this[type] && !s) {
			...
			while (next && !hs.isHsAnchor(next)) {
				if ((new RegExp('zoomer-'+ type)).test(next.className || null)) {
		...
		if (!this[type] && s) this[type] = hs.createElement('div', 
				{ className: 'zoomer-'+ type, innerHTML: s } );
...
focus : function() {
	...
	for (var i = 0; i < hs.expanders.length; i++) {
		if (hs.expanders[i] && i == hs.focusKey) {
			...
			blurExp.content.className += ' zoomer-'+ blurExp.contentType +'-blur';
	...
	this.content.className = 'zoomer-'+ this.contentType;
...
createOverlay : function (o) {
	...
	hs.setAttribs(overlay, {
		hideOnMouseOut: o.hideOnMouseOut,             //commented  by sajith on 9/04/10
		opacity: o.opacity || 1,
		hsPos: o.position,
		fade: o.fade
	});
...
getOverlays : function() {	
	...
	this.getNumber();
	if (this.heading && this.dragByHeading) this.heading.className += ' zoomer-move';
...
afterClose : function () {
	this.a.className = this.a.className.replace('zoomer-active-anchor', '');
...
getControls: function() {
	...
	for (var i = 0; i < buttons.length; i++) {
		this.btn[buttons[i]] = hs.getElementByClass(this.controls, 'li', 'zoomer-'+ buttons[i]);
